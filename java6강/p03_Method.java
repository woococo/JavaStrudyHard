...
public class p03_Method {
	static 	DecimalFormat k33_df = new DecimalFormat("#,###,###");	// 숫자 포맷을 k33_df로 정한다.
	static 	Calendar k33_calt = Calendar.getInstance(); // Calendar 클래스로 현재 시간을 가져온다.
	static 	SimpleDateFormat k33_sdtA = new SimpleDateFormat("YYYY-MM-dd HH:mm");	// 날짜 포맷을 k33_sdtA로 정한다.
	static 	SimpleDateFormat k33_sdtB = new SimpleDateFormat("YYYYMMdd");	// 날짜 포맷을 k33_sdtB로 정한다.
	static String[] k33_itemNames = { "해피홈 SAFE365 핸드크림", "(G)LH원형유리화병 10개묶음", "상투과자E", "더블링 나일론 깡 롤 휴지",
			"해피홈 SAFE 핸드워시", "농심 신라면 120g*5", "(대)국내산삼겹살찌개용", "오뚜기 진비빔면156g*5개", "GAP 죽장사과 4-6입", "순 유기농 바나나(봉)",
			"피코크 탄두리 닭가슴살", "(달콤)순살닭강정(g)", "DZ주니어양말3족C_BK", "DZ주니어양말3족C_WH", "프리미엄생연어초밥", "피코크 들깨미역국500g",
			"남양 맛있는우유GT 저지방", "녹차원흑당시럽10입", "진라면소컵(매)65g*6", "케라시스 앰플 트리트먼트", "푸르밀 미숫가루우유", "자일로스갈색설탕500g",
			"CJ 비비고칩 20G", "피코크 들깨미역국500", "흑원당 말차 밀크티", "1+ 등급란 10개입 특란", "동원양반볶음김치기획" };
	static int[] k33_price = { 75000, 69000, 5980200, 33000, 55000, 33800, 66100, 27500, 118000, 49800, 24800, 79200,
			49800, 49080, 71300, 39800, 44800, 49800, 32100, 69500, 29800, 19900, 19800, 39800, 44800, 34800, 790800 };
	static int[] k33_amount = { 2, 6, 8, 1, 20, 1, 5, 2, 99, 1, 4, 1, 3, 50, 2, 1, 1, 2, 1, 1, 2, 1, 1, 1, 1, 2, 1 };
	static boolean[] k33_taxfree = { false, false, false, false, false, false, true, false, true, true, false, false,
			false, false, false, false, true, false, false, false, false, false, false, false, false, true, false };
	static double k33_taxRate = 0.1; // 부가세율
	static int k33_sum = 0; // k33_price(단가) * k33_amount(수량) 값
	static int k33_totalTaxFreePrice = 0; // 면세 물품 합계
	static int k33_totalNetPrice = 0; // 과세물품 합계 = 과세물품 + 부가세
	static int k33_NetPrice = 0; // 과세물품
	static int k33_tax = 0; // 부가세
	static int k33_totalPrice = 0; // 결제 금액
	static int k33_point = 0; // 금회발생포인트
	static int k33_totalPoint = 5473; // 총 포인트

	public static void main(String[] args) {
		// TODO Auto-generated method stub
		int k33_itemCount = k33_itemNames.length; // 상품 항목이 몇개인지 배열 k33_itemNames의 길이로 판단 
		k33_TitlePrint(); // 영수증 맨 위 타이틀 인쇄
		k33_TimeStemp(); // 출력시간 인쇄
		k33_HeaderPrint(); // 헤더( 상품명, 단가, 수량, 금액) 인쇄
		
		for (int k33_i = 0; k33_i < k33_itemCount; k33_i++) { // 항목갯수만큼 항목인쇄
			k33_itemPrint(k33_i);
		}
		k33_totalPrint(); // 합계 세액등을 인쇄
	}

	public static void k33_TitlePrint() {
	...
	public static void k33_TimeStemp() {
	...
	public static void k33_itemPrint(int k33_i) {
		k33_sum = k33_price[k33_i] * k33_amount[k33_i]; // k33_sum은 k33_price(단가) * k33_amount(수량) 값으로 영수증 금액 부분에 출력되는
														// 변수이다.
		...
		if (k33_itemName.getBytes().length > k33_cutLength) {
			...
			for (char k33_ch : k33_itemName.toCharArray()) { // 문자 변수 k33_ch를 선언한다.
				k33_cnt += String.valueOf(k33_ch).getBytes().length; // k33_cnt에
																		// String.valueOf(k33_ch).getBytes().length 값을
																		// 계속 더한다
		...
		for (int k33_j = 0; k33_j < 16 - k33_itemNameLength; k33_j++) { // k33_j 가 0부터 (20 - k33_itemNameLength)까지 순차적으로
																		// 실행되는 for문을 생성한다.
	...
	public static void k33_totalPrint() {
		...
		System.out.printf("\n%22s %13d\n", "총 품목 수량", k33_itemNames.length); // 총 품목 수량을 출력하는데 배열 k33_itemNames의 길이 값으로
																				// 한다.
		System.out.printf("%23s%14s\n", "(*)면 세  물 품", k33_df.format(k33_totalTaxFreePrice)); // 면세 상품 가격총액을 포맷에
																								// 맞게(###,###,###)
																								// 출력해준다.
		System.out.printf("%23s%14s\n", "과 세  물 품", k33_df.format(k33_NetPrice)); // 과세 상품 가격총액을 포맷에 맞게(###,###,###)
																					// 출력해준다.
		...
		System.out.printf("%s%16s(%7s)\n", "누계(가용)포인트", k33_df.format(k33_totalPoint + k33_point),
				k33_df.format(k33_totalPoint)); 	// 총포인트와 적립전 포인트를 출력한다.
		System.out.printf("%s\n", "*신세계 포인트 유효기간은 2년입니다.");
		System.out.printf("%s\n", "-----------------------------------------");
		...
		System.out.printf("%39s\n", "||||||||||||||||||||||||||||||||||");
		System.out.printf("%15s%s\n", k33_sdtB.format(k33_calt.getTime()), "/00119861/00164980/31"); // 현재시각이 포함된 바코드번호를 출력해준다
	}
}
